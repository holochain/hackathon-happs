...
pub fn validate(op: Op) -> ExternResult<ValidateCallbackResult> {
    match op.to_type::<EntryTypes, LinkTypes>()? {
        OpType::StoreEntry(store_entry) => {
            match store_entry {
                OpEntry::CreateEntry { app_entry, action } => {
                    match app_entry {
                        EntryTypes::Submission(submission) => {
            ...
            match update_entry {
                OpUpdate::Entry {
                    ...
                    match (app_entry, original_app_entry) {
                        (
                            EntryTypes::Submission(submission),
                            EntryTypes::Submission(original_submission),
                        ) => {
        ...
        OpType::RegisterCreateLink {
            ...
            match link_type {
                LinkTypes::AllSubmissions => {
                    validate_create_link_all_submissions(
                        action,
                        base_address,
                        target_address,
                        tag,
        ...
        OpType::RegisterDeleteLink {
            ...
            match link_type {
                LinkTypes::AllSubmissions => {
                    validate_delete_link_all_submissions(
                        action,
                        original_action,
                        base_address,
                        target_address,
        ...
        OpType::StoreRecord(store_record) => {
            match store_record {
                ...
                OpRecord::UpdateEntry {
                    ...
                    match app_entry {
                        EntryTypes::Submission(submission) => {
                            let result = validate_create_submission(
                                EntryCreationAction::Update(action.clone()),
                                submission.clone(),
                            )?;
                            if let ValidateCallbackResult::Valid = result {
                                let original_submission: Option<Submission> = original_record
                                    .entry()
                                    .to_app_option()
                                    .map_err(|e| wasm_error!(e))?;
                                let original_submission = match original_submission {
